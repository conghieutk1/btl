...
private: System::Void button1_Click(System::Object^ sender, System::EventArgs^ e) {
	...
	//
	readEachLine("fileDATA.txt");
	//this->listView1->SelectedItems[count];
	//listView1->MoveListViewItems;
}
...
private: System::Void listView1_ItemSelectionChanged(System::Object^ sender, System::Windows::Forms::ListViewItemSelectionChangedEventArgs^ e) {
	...
	//listView1->Items->Remove(listView1->SelectedItems->Count[0]);
...
private: System::Void button3_Click(System::Object^ sender, System::EventArgs^ e) {
	...
	if (listView1->SelectedItems->Count > 0) {
		...
		try
		{
			StreamReader^ DataIn = File::OpenText("fileDATA.txt");
			String^ DataStr;
			int count = 0;
			while ((DataStr = DataIn->ReadLine()) != nullptr)
			{
				...
				temp = TaoData(stt, std::string(tuviettat), std::string(tugoc));
				//push từng hàng vào vector DATABASE
			...
			for (int i = 0; i < DATABASE.size(); i++) {
				if (i > _stt - 1) {//khi stt lớn hơn đối tượng đc chọn, stt trong file text sẽ trừ 1
					fp << "[" << (DATABASE[i].stt)-1 << "] " << DATABASE[i].tvt << " " << DATABASE[i].tg << ".\n";
					continue;
				}
				if (i == _stt - 1) { // khi stt trong file text == stt đối tượng đc chọn thì k ghi vector này lại vào file
					// continue;
				}
				else {
					fp << "[" << DATABASE[i].stt << "] " << DATABASE[i].tvt << " " << DATABASE[i].tg << ".\n";
				}

			}
			fp.close();
			DATABASE.clear();
			//load lại bảng listView
			readEachLine("fileDATA.txt");
		}
		catch (Exception^ e)
		{
			if (dynamic_cast<FileNotFoundException^>(e))
				MessageBox::Show("File fileDATA.txt not found");
		}
	}
	else {
		MessageBox::Show("Vui long chon doi tuong can xoa");
	}
}
//Nút Sửa
private: System::Void button2_Click(System::Object^ sender, System::EventArgs^ e) {
	...
	if (listView1->SelectedItems->Count > 0) {
		...
		try
		{
			...
			DataIn->Close();
			// sửa đối tượng đc chọn bằng thao tác trên vector
		...
		MessageBox::Show("Vui long chon doi tuong can chinh sua");
	}
}
