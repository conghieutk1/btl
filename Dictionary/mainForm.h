...
namespace Dictionary {
	...
	public:
		...
		void readFile(String^ fileName) {
			...
			//Sắp xếp vector ABC theo đúng quy luật
			for (int i = 0; i < DATABASE_abc.size(); i++) {
				for (int j = i + 1; j < DATABASE_abc.size(); j++) {
		...
		bool button4WasClicked = false; // phần design: hàm textBox1_Leave
...
private: System::Void button1_Click(System::Object^ sender, System::EventArgs^ e) {
	...
	//Nếu cả vector không có tvt của từ thứ i > tvt của từ mới
	...
	//Sau khi thêm v
	for (int i = 0; i < DATABASE_abc.size(); i++) {
		if (DATABASE_abc[i].abc > DATABASE_abc[p].abc) {
			(DATABASE_abc[i].abc)++;
		}
	}
...
private: System::Void textBox1_Leave(System::Object^ sender, System::EventArgs^ e) {
	textBox1->ForeColor = System::Drawing::Color::FromArgb(static_cast<System::Int32>(static_cast<System::Byte>(200)), static_cast<System::Int32>(static_cast<System::Byte>(200)),
		static_cast<System::Int32>(static_cast<System::Byte>(200)));
	if (button4WasClicked = true) {

	}
	else {
		textBox1->Text = "Search words here...";
	}
}	
...
private: System::Void button4_Click(System::Object^ sender, System::EventArgs^ e) {
	...
	else tt = SearchTG(DATABASE_123, std::string(TCT)); // Tim theo tu goc
	...
	if (tt == -1) {
		...
		if (type_sort == 0) {
			//std::string test1 = std::to_string(tt);
			//MessageBoxA(NULL, tt.c_str(), "testx", MB_OK);
